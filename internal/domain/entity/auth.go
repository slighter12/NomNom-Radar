...
type Authentication struct {
	...
	PasswordHash   string       // Stores the hashed password, only used when the Provider is "email".
...
type RefreshToken struct {
	...
	FamilyID   uuid.UUID  // Groups tokens that belong to the same login session lineage.
	IsRevoked  bool       // Indicates whether the token has been rotated out or invalidated.
	ReplacedBy *uuid.UUID // Points to the token that replaced this token during rotation.
