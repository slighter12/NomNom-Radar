...
- Linking token payload exposure: linking JWT claims currently include onboarding draft fields such as `storeName` and `businessLicense`. This is acceptable for the short-lived signed-token flow, but move to server-side pending-link state if those fields are later treated as sensitive or compliance-restricted data.
